...
def delete_usuario(user_id):
    ...
    try:
        ...
        if usuario_para_deletar.username == usuario_logado:
            return jsonify({'erro': 'Não é permitido excluir o próprio usuário logado.'}), 403
...
def reordenar_prioridade():
    ...
    nova_prioridade = data.get('nova_prioridade')

    if not pedido_id or not isinstance(nova_prioridade, int) or nova_prioridade <= 0:
    ...
    try:
        with engine.connect() as conn:
            with conn.begin():
                result = conn.execute(
                    text("SELECT prioridade FROM pedidos_tb WHERE id = :id"),
                    {"id": pedido_id}
                ).scalar_one_or_none()
                ...
                prioridade_antiga = result

                if nova_prioridade == prioridade_antiga:
                    return jsonify({'mensagem': 'O pedido já está nesta posição.'})

                if nova_prioridade < prioridade_antiga:
                    conn.execute(
                        text("""
                            UPDATE pedidos_tb SET prioridade = prioridade + 1
                            WHERE prioridade >= :nova AND prioridade < :antiga
                        """),
                        {"nova": nova_prioridade, "antiga": prioridade_antiga}
                    )
                else:
                    conn.execute(
                        text("""
                            UPDATE pedidos_tb SET prioridade = prioridade - 1
                            WHERE prioridade > :antiga AND prioridade <= :nova
                        """),
                        {"antiga": prioridade_antiga, "nova": nova_prioridade}
                    )
                
                conn.execute(
                    text("UPDATE pedidos_tb SET prioridade = :nova WHERE id = :id"),
                    {"nova": nova_prioridade, "id": pedido_id}
                )

        return jsonify({'mensagem': 'Prioridade atualizada com sucesso!'}), 200

    except Exception as e:
        print(f"Erro ao reordenar prioridade: {e}")
        return jsonify({'erro': 'Ocorreu um erro interno no servidor ao reordenar.'}), 500
